...
void runTestLesson2Task2() {
  ...
  testWidgets('module2', (WidgetTester tester) async {
    ...
    expect(
      ...
      findsOneWidget,
      reason: "ОШИБКА! На странице CreateMemePage невозможно найти единственную кнопку с текстом '$addTextText'",
    );
    ...
    final draggableMemeTextWithEnteredTextFinder = find.widgetWithText(MemeTextWidget, firstText);
    ...
    final draggableMemeTextWithEnteredText = tester.widget<MemeTextWidget>(draggableMemeTextWithEnteredTextFinder);

    print("Проверяем что параметр offset в объекте memeTextWithOffset внутри DraggableMemeText не равен null");
    expect(
      draggableMemeTextWithEnteredText.text.offset,
      isNotNull,
      reason: "ОШИБКА! Параметр параметр offset в объекте memeTextWithOffset внутри DraggableMemeText равен null",
    );

    final savedOffset = draggableMemeTextWithEnteredText.text.offset!;
    ...
    expect(
      ...
      reason: "ОШИБКА! Позиция виджета из параметра memeTextWithOffset не совпадает с реальной позицией виджета",
    );
