  ...
  testWidgets('module4', (WidgetTester tester) async {
    await tester.runAsync(() async {
      ...
      final firstMemeGlobal = Meme(
        ...
        photo: "$docsMemesPath$ps$firstImageName",
      );

      final saveMemeInteractor = SaveMemeInteractor.getInstance();
      final memeRepository = MemeRepository.getInstance();
      ...
      final secondMemeGlobal = Meme(
        ...
        photo: "$docsMemesPath$ps$secondImageName",
      );
      print("Сохраняем еще один мем через SaveMemeInteractor");
      saveMemeInteractor.saveMeme(
        id: secondId,
        texts: secondTextWithPositions,
        photo: secondImagePath,
      );
      ...
      final thirdMemeGlobal = Meme(
        ...
        photo: "$docsMemesPath$ps$thirdImageName",
      );
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: CreateMemePage(id: thirdId, photo: thirdImagePath),
        ),
      );

      final saveButtonFinder = find.byWidgetPredicate((widget) => widget is Icon && widget.icon == Icons.save);

      print("Находим единственную кнопку с иконкой Icons.save");
      expect(
        saveButtonFinder,
        findsOneWidget,
        reason: "ОШИБКА! На странице CreateMemePage невозможно найти единственный виджет с иконкой Icons.save",
      );
